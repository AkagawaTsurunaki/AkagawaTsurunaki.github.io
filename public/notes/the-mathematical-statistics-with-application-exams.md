...
> [!WARNING]
> 题目没有指出灯泡寿命服从于什么分布，且样本数量较小，我最开始使用了秩和检验，因为其在应用时可以不考虑被研究对象为何种分布以及分布是否已知. 但是，我的朋友指出有一道类似的例题使用的单因素方差分析，但是修改了部分数据，并删除了丙、丁等数据. 根据我进一步的查阅，秩和检验更适合需要配对成组的场景，因此这道题应该使用单因素方差分析. 

**解：**

计算样本均值 $\bar{x}=1516.11$，$\bar{x}_1 = 1497$，$\bar{x}_2 = 1540$，$s_1^2 = 1280/4$，$s_2^2 = 7400/3$. 
...
$$
TSS = \sum_{i=1}^{r} \sum_{j=1}^{n_i} (x_{ij} - \bar{x})^2 = 12788.89
\\
RSS = \sum_{i=1}^{r} (n_i - 1) s_i^2 = 4 \times \frac{1280}{4} + 3 \times \frac{7400}{3} = 8680
\\
CSS = TSS - RSS = 12788.89 - 8680 = 4108.89
$$
...
$$
F = \frac{7}{1} \times \frac{4108.89}{8680} = 3.31
$$

查表可知 $F_{\alpha}(r-1,n-r) = F_{0.05}(1,7)=6.61$.

因为 $F=3.31<6.61=F_{0.05}(1,7)$，所以不拒绝原假设，认为灯丝配料方案对灯泡寿命没有显著影响.
