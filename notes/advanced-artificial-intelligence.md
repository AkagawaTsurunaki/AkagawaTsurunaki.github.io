...
1. 判断： (1) $S \perp\!\!\!\perp V$；(2) $S \perp\!\!\!\perp M \mid C$；(3) $P \perp\!\!\!\perp V \mid C$；
2. 计算条件概率 $P(-m \mid +s, +c, +f, -cc, +v)$. 
...
对于 $P \leftarrow C \rightarrow V$，$C$ 是 tail-to-tail 节点，$C$ 在条件集 $\{C\}$ 中，$C$ 阻塞，所以 $P \perp\!\!\!\perp V \mid C$ 为 True.
...
根据题意，有 $P(-m) = 0.2$，$P(+s) = 0.3$， $P(+cc) = 0.1$，$P(-c \mid +s, -m) = 0.2$，$P(-f \mid +cc, -m) = 0.05$，$P(+v \mid -c, -f) = 0.1$，所以

$$
P(-m, +s, -c, -f, +cc, +v) = 0.2 \times 0.3 \times 0.1 \times 0.2 \times 0.05 \times 0.1 = 6 \times 10^{-6}
$$

根据题意，有 $P(+m) = 0.8$，$P(+s) = 0.3$， $P(+cc) = 0.1$，$P(-c \mid +s, +m) = 0.5$，$P(-f \mid +cc, +m) = 0.4$，$P(+v \mid -c, -f) = 0.1$，所以
$$
P(+m, +s, -c, -f, +cc, +v) = 0.8 \times 0.3 \times 0.1 \times 0.5 \times 0.4 \times 0.1 = 4.8 \times 10^{-4}
$$
最后，可得
$$
P(-m \mid +s, +c, +f, -cc, +v) = \dfrac{6 \times 10^{-6}}{6 \times 10^{-6} + 4.8 \times 10^{-4}} = 0.012345679
$$
...
$$
a_{11} = \sigma(z_{11}) = \frac{1}{1 + e^{-4}} = 0.98 \\
a_{12} = \sigma(z_{12}) = \frac{1}{1 + e^{-4}} = 0.98
$$
