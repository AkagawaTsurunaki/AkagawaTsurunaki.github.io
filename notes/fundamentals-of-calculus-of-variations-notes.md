...
**$n$ 阶（级）距离：**两函数的 $0$ 至 $n$ 阶导数之差的绝对值中最大的那个数.
...
$$
\Delta J = L[y(x), \delta y] + \underbrace{d[y(x), \delta y]}_{\text{$\delta y$ 的高阶无穷小量}}
$$
...
- $\delta\left(\dfrac{F_1}{F_2}\right) = \dfrac{F_2\delta F_1 - F_1\delta F_2}{F_2^2}$（上变下不变，减，下变上不变，分母取平方）
...
可动边界的最简泛函的极值曲线 $y=y(x)$ 左右端点分别在 $y=\varphi(x)$ 与 $y=\psi(x)$ 上待定，则横街条件
...
连续 $\xlongequal{充要条件}$ 有界： 线性算子 $T$ 连续当且仅当 $T$ 有界.
